## ---------------------------
##
## Script name: ntl-prod-cons-crystallake.R
##
## Purpose of script: munging data from crytal mix project for SPARC project
##
...
chl <-read.csv(infile1,header=F,skip=1,sep=",",quot='"',
               col.names=c("lakeid", "year4", "daynum","sampledate", 
                           "rep", "sta", "depth",     
                           "chlor", "phaeo","flagchlor", "flagphaeo"), 
               check.names=TRUE)
...
chl_wide = pivot_wider(chlorophyll, 
                       ...
                       names_prefix = 'chl',
                       values_from = chlor) %>%
  mutate(chl3 = case_when(is.na(.$chl3) ~ chl2, TRUE ~ chl3),
         chl8 = case_when(is.na(.$chl8) ~ chl7, TRUE ~ chl8),
         chl10 = case_when(is.na(.$chl10) ~ chl9, TRUE ~ chl10)) %>%
  select(-chl2, -chl7, -chl9)
...
# groups from this source: http://cfb.unh.edu/cfbkey/html/groups.html
# write.csv(unique(crystal_zoops$species_name), file = 'Crystal Zoop Species.csv')

#read the species grouping file back in merge with data frame
groups = read.csv("Crystal Zoop Species.csv") %>%
  rename(species_name = species)
#Merge the two data frames
crystal_zoops = left_join(crystal_zoops, groups, by = c("species_name"))
...
ntl_prod_cons$mix_dist = NA
ntl_crystal = ntl_prod_cons %>%
  mutate(mix_dist = case_when(year4 < 2012 ~ "pre",
                              year4 == 2012 ~ 'mix',
                              year4 == 2013 ~ 'mix',
                              year4 >= 2014 ~ "post"))

#Save the file
# write.csv(ntl_crystal, file = "NTL_CrystalLake_Mixing.csv")
